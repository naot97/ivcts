...
class SpendTimePlanModel extends Model {
    private $updateBy;
    ...
    public function insSpendTimePlan($params) {
        if(($spendTimePlan = SpendTimePlan::findOne(['sectionID' => $params['sectionID'], 
                                                         ...
                                                         'enabled' => 0])) != null) {
            $params['spendTimeID'] = $spendTimePlan->spendTimeID;
            $params['enabled'] = 1;
            return $this->updSpendTimePlan($params);
        }                
        ...
        try {
            $spendTimePlan = $this->mergeData(new SpendTimePlan(), $params);  
            return $spendTimePlan->save();
    ...
    public function updSpendTimePlan($params) {
        $messError = $this->checkValid($params);
        if($messError != '') {
            throw new \Exception($messError);
        }
        ...
        try {
            $spendTimePlan = $this->mergeData(SpendTimePlan::findOne($params['spendTimeID']), $params);          
            return $spendTimePlan->save();
        } catch (\yii\db\Exception $exc) {
            switch ($exc->getCode()) {
                case 23000:  
    ...
    public function delSpendTimePlan($params) {
        $spendTimePlan = $this->mergeData(SpendTimePlan::findOne($params['spendTimeID']), $params);  
        return $spendTimePlan->save();
    }
    
    public function getSpendTimePlanBy($params) {
        $query = 'SELECT * '
    ...
    public function getLogs($params) {
        $query = 'SELECT ID, spendTimeID, sectionID, employeeID, projectID, periodID, rankID, typeID, effort, ot, updateTime, action, note, '
               ...
               . '    ELSE \'\' '
               . 'END AS \'updateBy\' ' 
               . 'FROM ' . substr(SpendTimePlan::tableName(), 0, -3) . 'l}} '
